    ...
    <SafeAreaView style={styles.container} edges={safeAreaEdges}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          {/* Laundry & Usage Tracking Section */}
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Usage & Laundry Tracking</Text>
            
            <View style={styles.trackingRow}>
              <View style={styles.trackingItem}>
                <Text style={styles.trackingLabel}>Times Worn</Text>
                <Text style={styles.trackingValue}>{localItem.wearCount || 0}</Text>
                <Pressable
                  style={[styles.trackingButton, isProcessing && styles.trackingButtonDisabled]}
                  onPress={async () => {
                    if (isProcessing) return;
                    try {
                      const { clothingApi } = await import("../services/api");
                      await clothingApi.updateUsage(id, "worn");
                      // Reload item to get updated data
                      const updated = await clothingApi.getById(id);
                      setLocalItem({
                        ...localItem,
                        wearCount: updated.wearCount || 0,
                        lastWorn: updated.lastWorn,
                      });
                    } catch (error) {
                      Alert.alert("Error", "Failed to update usage tracking");
                    }
                  }}
                  disabled={isProcessing}
                >
                  <MaterialCommunityIcons name="check-circle" size={20} color={colors.icon_stroke} />
                  <Text style={styles.trackingButtonText}>Mark as Worn</Text>
                </Pressable>
                {localItem.lastWorn && (
                  <Text style={styles.trackingDate}>
                    Last worn: {new Date(localItem.lastWorn).toLocaleDateString()}
                  </Text>
                )}
              </View>

              <View style={styles.trackingItem}>
                <Text style={styles.trackingLabel}>Times Washed</Text>
                <Text style={styles.trackingValue}>{localItem.washCount || 0}</Text>
                <Pressable
                  style={[styles.trackingButton, isProcessing && styles.trackingButtonDisabled]}
                  onPress={async () => {
                    if (isProcessing) return;
                    try {
                      const { clothingApi } = await import("../services/api");
                      await clothingApi.updateUsage(id, "washed");
                      // Reload item to get updated data
                      const updated = await clothingApi.getById(id);
                      setLocalItem({
                        ...localItem,
                        washCount: updated.washCount || 0,
                        lastWashed: updated.lastWashed,
                      });
                    } catch (error) {
                      Alert.alert("Error", "Failed to update laundry tracking");
                    }
                  }}
                  disabled={isProcessing}
                >
                  <MaterialCommunityIcons name="washing-machine" size={20} color={colors.icon_stroke} />
                  <Text style={styles.trackingButtonText}>Mark as Washed</Text>
                </Pressable>
                {localItem.lastWashed && (
                  <Text style={styles.trackingDate}>
                    Last washed: {new Date(localItem.lastWashed).toLocaleDateString()}
                  </Text>
                )}
              </View>
            </View>
